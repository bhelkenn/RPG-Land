...
public:
	...
	void equipment_menu() {
		...
		cout << "3. View detailed information about an item" << endl;
				...
				case 1:
					if (count > 0) {
						...
						while (true) {
							...
							cout << count + 1 << ". Cancel request" << endl;
							...
							else if (equip_choice == count) {goto equipment_menu_start;}
	...
	void view_attrib_description() {
		...
		while (true) {
			Clear();
			cout << "What attribute would you like to learn more about?" << endl;
	...
	void view_inventory() {
		...
		if (items_in_inventory == 0) {
			...
			for (int i = 0; i < items_in_inventory; i++) {
				cout << i + 1 << ". " << inventory[i].get_name() << " x" << inventory[i].get_quantity();
	...
	void attribs_menu() {
		...
		if (points == 0) {cout << "(R)eturn without doing anything" << endl;}
		...
		while (true) {
			...
			if (choice == 'L' || choice == 'l') {
				...
				if (points > 0) {allocate_points();}
				goto attribs_menu_start;
			}
			else if (choice == 'R' || choice == 'r') {
				if (points == 0) {return;}
				else {
					cout << "You must spend your points before continuing!" << endl;
					goto attribs_menu_start;
				}
			}
			else {cout << "Invalid entry. ";}
			...
			if (choice < SPELL_COUNT) {
				if (spellbook[choice].get_cost() > current_mana) {
					...
					if (spellbook[choice].get_type() == "Offensive") {
						creature.take_spell_dmg(spellbook[choice].get_magnitude(), accuracy, crit);
					}
			...
			if (choice < size_of_inventory) {
				if (player->get_gold() >= inventory[choice].get_buy_value()) {
					...
					while (true) {
						...
						cout << "2. View detailed information" << endl;
						...
						else if (selection_choice == 3) {goto buy_start;}
						else {
							cout << "Invalid entry. ";
							Pause();
						}
...
public:
	...
	void menu(Player* &player) {
		...
		while (true) {
			...
			else if (choice == 3) {repair(player);}
			else if (choice == 4) {
...
void fight_creature(Player* &player) {
	...
	cout << "What would you like to do?" << endl;
	cout << "1. Attack" << endl;
	if (player->get_player_class() == "Mage") {cout << "2. Cast a spell" << endl;}
	else {cout << "2. Use your class ability" << endl;}
