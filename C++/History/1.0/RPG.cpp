...
const int ATTRIB_BONUS = 5;
const int CLASS_BONUS = 10;
const int LEVEL_INCREMENT = 50;
...
public:
	...
	void view_stats() {
		...
		cout << "Mana: " << current_mana << "/" << max_mana << endl << endl;
		cout << "Experience: " << exp << "/" << exp_to_next_level << endl;
...
private:
	...
	void init_attribs() {
		...
		points = 20;
		exp = items_in_inventory = armor_rating = 0;
		level = 1;
		gold = 300;
	}
...
public:
	...
	Player menu(Player player) {
		system("CLS");
		cout << "You enter the blacksmith. The proprietor notices you enter." << endl;
		menu_start:
		cout << "Blacksmith: 'Welcome to my shop! What would you like?'" << endl;
		...
		while (true) {
			switch (choice) {
				...
				case 'L':
				case 'l':
					cout << "You decide to leave the blacksmith's shop. "
					<< "The proprietor scowls at your retreating back." << endl;
					system("PAUSE");
					return player;
					break;
				default:
...
private:
	...
	void display_inventory() {
		system("CLS");
		cout << "Blacksmith: 'Here's what I have to offer:'" << endl << endl;
	...
	Player sell(Player player) {
		...
		if (player.get_items_in_inventory() == 0) {
			...
			cout << "Blacksmith: 'What would you like to sell to me?' " << endl << endl;
...
public:
	...
	Player menu(Player player) {
		system("CLS");
		cout << "You enter the inn. A buxom maiden notices your entrance and sashays over." << endl;
		menu_start:
		cout << "Innkeeper: 'Welcome to my inn!'" << endl;
		...
		while (true) {
			switch (choice) {
				...
				case 'L':
				case 'l':
					cout << "You decide to leave, possibly put off by the innkeeper's come-hither stares. "
					<< "She pouts after you as you leave." << endl;
					system("PAUSE");
					return player;
					break;
				default:
...
private:
	...
	void display_inventory() {
		system("CLS");
		cout << "Innkeeper: 'Here's what I have to offer:'" << endl << endl;
		for (int i = 0; i < size_of_inventory; i++) {
			cout << i + 1 << ". " << inventory[i].get_name() << " - "
			<< inventory[i].get_buy_value() << " gold" << endl;
		}
	...
	Player sell(Player player) {
		...
		if (player.get_items_in_inventory() == 0) {
			...
			Item *player_inventory = player.get_inventory();
			cout << "Innkeeper: 'What would you like to do to - I mean, sell me?' " << endl << endl;
			for (int i = 0; i < player.get_items_in_inventory(); i++) {
				cout << i + 1 << ". " << player_inventory[i].get_name() << " x" << player_inventory[i].get_quantity()
				<< " - " << player_inventory[i].get_sell_value() << " gold" << endl;
			}
		...
		switch (choice) {
			...
			case 's':
				if (player.get_player_class() == "Mage") {
					spells = player.get_spellbook();
					cout << "\nWhat spell would you like to cast?" << endl;
					player.view_spells();
					int choice;
					cin >> choice;
					choice--;
					cout << "You cast " << spells[choice].get_name() << "!" << endl;
					if (spells[choice].get_type() == "Offensive") {
						creature.take_spell_dmg(spells[choice].get_magnitude(), player.get_accuracy(), player.get_crit());
						if (creature.is_dead()) {
							cout << "You've killed the " << creature.get_name() << "!" << endl;
							player.gain_exp(creature.get_exp());
							player.add_gold(creature.get_money());
					...
					if (spells[choice].get_type() == "Healing") {
						player.restore_health(spells[choice].get_magnitude());
						cout << "You've restored " << spells[choice].get_magnitude() << " health!" << endl;
					}
					if (spells[choice].get_type() == "Defensive") {
						player.boost_def(spells[choice].get_magnitude());
						cout << "You've boosted your defense by " << spells[choice].get_magnitude() << endl;
					}
					player.lose_mana(spells[choice].get_cost());
					player.take_damage(creature.get_atk(), creature.get_accuracy(), creature.get_crit());
					if (player.is_dead()) {
						player.dies();
						return player;
